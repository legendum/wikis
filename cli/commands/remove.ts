...
import { removeProject, getApiUrl, getAccountKey } from "../lib/config";
...
export default async function remove(_args: string[]) {
  ...
  // Confirm
  const answer = await prompt("Are you sure? This will permanently delete the wiki folder and remove it from the server. (y/N): ");
  ...
  const config = yaml.load(readFileSync(configPath, "utf8")) as { name: string };
