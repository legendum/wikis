...
describe("findChangedPages", () => {
  ...
  it("returns empty when source is older than wiki page", () => {
    ...
    ).run(wikiId, "src/app.ts", "code", "abc", "overview.md", "2026-01-01T00:00:00Z");
  ...
  it("detects page needing update when source is newer", () => {
    db.prepare(
      "INSERT INTO source_files (wiki_id, path, content, hash, wiki_paths, modified_at) VALUES (?, ?, ?, ?, ?, ?)"
    ).run(wikiId, "src/app.ts", "code v2", "abc2", "overview.md", "2026-01-03T00:00:00Z");
  ...
  it("handles comma-separated wiki_paths correctly", () => {
    db.prepare(
      "INSERT INTO source_files (wiki_id, path, content, hash, wiki_paths, modified_at) VALUES (?, ?, ?, ?, ?, ?)"
    ).run(wikiId, "README.md", "readme", "abc", "overview.md,setup.md,api.md", "2026-01-05T00:00:00Z");
  ...
  it("does not match partial path names", () => {
    ...
    db.prepare(
      "INSERT INTO source_files (wiki_id, path, content, hash, wiki_paths, modified_at) VALUES (?, ?, ?, ?, ?, ?)"
    ).run(wikiId, "src/api.ts", "code", "abc", "api.md", "2026-01-05T00:00:00Z");
    ...
    db.prepare(
      "INSERT INTO wiki_files (wiki_id, path, content, hash, modified_at) VALUES (?, ?, ?, ?, ?)"
    ).run(wikiId, "api-overview.md", "# API Overview", "ghi", "2026-01-01T00:00:00Z");
  ...
  it("handles multiple source files pointing to same wiki page", () => {
    db.prepare(
      "INSERT INTO source_files (wiki_id, path, content, hash, wiki_paths, modified_at) VALUES (?, ?, ?, ?, ?, ?)"
    ).run(wikiId, "src/auth.ts", "code", "abc", "auth.md", "2026-01-01T00:00:00Z");

    db.prepare(
      "INSERT INTO source_files (wiki_id, path, content, hash, wiki_paths, modified_at) VALUES (?, ?, ?, ?, ?, ?)"
    ).run(wikiId, "src/session.ts", "code", "def", "auth.md", "2026-01-10T00:00:00Z");
  ...
  it("ignores source files where wiki page does not exist yet", () => {
    db.prepare(
      "INSERT INTO source_files (wiki_id, path, content, hash, wiki_paths, modified_at) VALUES (?, ?, ?, ?, ?, ?)"
    ).run(wikiId, "src/new.ts", "code", "abc", "new-feature.md", "2026-01-10T00:00:00Z");
