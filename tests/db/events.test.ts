...
function recordEvent(db: Database, wikiId: number | null, type: string, count = 1): void {
  db.prepare("INSERT INTO events (wiki_id, type, count) VALUES (?, ?, ?)").run(wikiId, type, count);
}
