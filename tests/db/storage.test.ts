...
function upsertFile(db: Database, wikiId: number, path: string, content: string, modified: string): void {
...
describe("storage", () => {
  ...
  it("upserts new files", () => {
    ...
    const row = db.prepare("SELECT content FROM wiki_files WHERE wiki_id = ? AND path = ?").get(wikiId, "page.md") as { content: string };
  ...
  it("updates existing files on conflict", () => {
    ...
    const row = db.prepare("SELECT content, modified_at FROM wiki_files WHERE wiki_id = ? AND path = ?").get(wikiId, "page.md") as { content: string; modified_at: string };
  ...
  it("lists files ordered by path", () => {
    ...
    const rows = db.prepare("SELECT path FROM wiki_files WHERE wiki_id = ? ORDER BY path").all(wikiId) as { path: string }[];
    expect(rows.map((r) => r.path)).toEqual(["a.md", "m.md", "z.md"]);
  });

  it("deletes files", () => {
    upsertFile(db, wikiId, "page.md", "content", "2026-01-01");
    db.prepare("DELETE FROM wiki_files WHERE wiki_id = ? AND path = ?").run(wikiId, "page.md");
    const row = db.prepare("SELECT * FROM wiki_files WHERE wiki_id = ? AND path = ?").get(wikiId, "page.md");
    expect(row).toBeNull();
  });
  ...
  it("builds manifest with hash and modified", () => {
    ...
    const rows = db.prepare("SELECT path, hash, modified_at FROM wiki_files WHERE wiki_id = ?").all(wikiId) as { path: string; hash: string; modified_at: string }[];
    const manifest: Record<string, { hash: string; modified: string }> = {};
    for (const row of rows) manifest[row.path] = { hash: row.hash, modified: row.modified_at };
