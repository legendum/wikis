...
function recordUpdate(db: Database, wikiId: number, path: string, summary: string): void {
  db.prepare("INSERT INTO wiki_updates (wiki_id, path, summary) VALUES (?, ?, ?)").run(wikiId, path, summary);
}

function getPageUpdates(db: Database, wikiId: number, path: string, limit = 5): { summary: string; created_at: string }[] {
  return db
    .prepare("SELECT summary, created_at FROM wiki_updates WHERE wiki_id = ? AND path = ? ORDER BY created_at DESC LIMIT ?")
...
describe("wiki_updates", () => {
  ...
  beforeEach(() => {
    ...
    wikiId = (db.prepare("SELECT id FROM wikis WHERE name = ?").get("test") as { id: number }).id;
  });
  ...
  it("records and retrieves updates for a page", () => {
    recordUpdate(db, wikiId, "architecture.md", "Created: covers system architecture");
  ...
  it("returns updates in reverse chronological order", () => {
    ...
    db.prepare("INSERT INTO wiki_updates (wiki_id, path, summary, created_at) VALUES (?, ?, ?, ?)").run(wikiId, "api.md", "Created: API overview", "2026-01-01 00:00:00");
    db.prepare("INSERT INTO wiki_updates (wiki_id, path, summary, created_at) VALUES (?, ?, ?, ?)").run(wikiId, "api.md", "Updated: added auth section", "2026-01-02 00:00:00");
    db.prepare("INSERT INTO wiki_updates (wiki_id, path, summary, created_at) VALUES (?, ?, ?, ?)").run(wikiId, "api.md", "Updated: added rate limiting", "2026-01-03 00:00:00");
  ...
  it("isolates updates between different wikis", () => {
    ...
    const otherWikiId = (db.prepare("SELECT id FROM wikis WHERE name = ?").get("other") as { id: number }).id;
